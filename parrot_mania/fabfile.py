...
${prefix}_DATABASE_NAME=parrot_mania
${prefix}_DATABASE_USER=parrot_mania
        ...
        "letsencrypt": [
            {
                ...
                "filename": "letsencrypt.parrot_mania.conf",
        ...
        "nginx": [
            {
                ...
                "filename": "app.parrot_mania.include",
                "remote_path": "/etc/nginx/conf.d/%(filename)s"
            },
            {
                "pattern": "common.parrot_mania.node.include",
                "filename": "common.parrot_mania.node.include",
                "remote_path": "/etc/nginx/conf.d/%(filename)s"
            },
            {
                "pattern": "common.parrot_mania.django.include",
                "filename": "common.parrot_mania.django.include",
                "remote_path": "/etc/nginx/conf.d/%(filename)s"
            },
            {
                "pattern": "app.parrot_mania.proxy_django.include",
                "filename": "app.parrot_mania.proxy_django.include",
                "remote_path": "/etc/nginx/conf.d/%(filename)s"
            },
            {
                "pattern": "app.parrot_mania.proxy_node.include",
                "filename": "app.parrot_mania.proxy_node.include",
                "remote_path": "/etc/nginx/conf.d/%(filename)s"
            },
            {
                "pattern": "app.parrot_mania.proxy.include",
                "filename": "app.parrot_mania.proxy.include",
                "remote_path": "/etc/nginx/conf.d/%(filename)s"
            },
            {
                "pattern": "%(target)s.ssl",
                "filename": "ssl.parrot_mania.include",
                "remote_path": "/etc/nginx/conf.d/%(filename)s"
            },
            {
                "default_site": True,
                "pattern": "%(target)s.conf",
                "filename": "parrot_mania",
...
def test():
    ...
    defaults()
    env.node_site = 'parrot-mania.test.parrotmania.com'
    env.django_site = 'office.parrot-mania.test.parrotmania.com'
    env.hosts = ['test.parrotmania.com']
...
def live():
    ...
    env.node_site = 'parrotmania.com'
    env.django_site = 'office.parrotmania.com'
    env.hosts = ['production.parrotmania.com']
...
def setup_server(id=None):
    ...
    django_settings_file = env.code_dir + '/parrot_mania/django.env'
    ...
    # Create database
    sudo('echo "CREATE DATABASE parrot_mania; '
         '      CREATE USER parrot_mania WITH password \'{db_password}\'; '
         '      GRANT ALL PRIVILEGES ON DATABASE parrot_mania to parrot_mania;" '
         '| docker exec -i postgres-{postgres_version} psql -U postgres'.format(db_password=db_password,
                                                                                postgres_version=env.postgres_version))
    # Create log dir
    sudo('mkdir -p /var/log/parrot_mania/')
    ...
    with cd(env.code_dir):
        sudo('cp deploy/logrotate.conf /etc/logrotate.d/parrot_mania')
...
def add_secret_keys(component=None):
    ...
    if component == 'node':
        ...
        remote_path = env.code_dir + '/parrot_mania/django.env'
...
def docker_compose_run(service, cmd='', name='parrot_mania_tmp'):
...
def ensure_docker_networks():
    ...
    ensure_docker_network_exists('parrot_mania_default', [], internal=False)
    ensure_docker_network_exists('parrot_mania_nginx', ['nginx'])
    ensure_docker_network_exists('parrot_mania_postgres', ['postgres-{postgres_version}'.format(
        postgres_version=env.postgres_version)])
...
def ensure_local_py_exists():
    ...
    django_local_settings = env.code_dir + '/parrot_mania/settings/local.py'
